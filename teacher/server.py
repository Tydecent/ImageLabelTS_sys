...
UPLOAD_META_FILE = "upload_meta.json"   # 存储 {学生名: {图片基本名: {"hash": "md5", "last_upload": "ISO时间"}}}
...
def push():
    ...
    # 计算哈希
    file_hash = compute_md5(save_path)
    ...
    # 更新该学生的元数据
    if name not in upload_meta:
        upload_meta[name] = {}
    upload_meta[name][base_name] = {
        "hash": file_hash,
...
def status():
    ...
    if os.path.isdir(student_dir):
        ...
        if os.path.exists(UPLOAD_META_FILE):
            with open(UPLOAD_META_FILE, 'r', encoding='utf-8') as f:
                upload_meta = json.load(f)
            student_meta = upload_meta.get(name, {})
            for img in assignments[name]:
                base = os.path.splitext(img)[0]
                if base in student_meta:
                    uploaded_details[base] = {
                        "hash": student_meta[base]["hash"],
                        "last_upload": student_meta[base]["last_upload"]
                    }
    ...
    return jsonify({
        ...
        "total": total,
        "assigned_images": assignments.get(name, []),
        "uploaded_details": uploaded_details   # 新增字段
    })
